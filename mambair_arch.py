...
class SS2D(nn.Module):
    def __init__(
        ...
        self.query_conv = nn.Conv2d(self.d_inner, self.d_inner, 1)
        self.key_conv = nn.Conv2d(self.d_inner, self.d_inner, 1)

        self.concat_conv = nn.Conv2d(4 * d_model, 2*d_model, kernel_size=3, padding=1, stride=1)

        self.smooth = nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        scores = torch.matmul(query, key) / scale_factor  # (B, W, H, H)
        # Reshape here to B,1,W,H*H -> B,H*W,1,W
        # convolution C channels nn.Conv2d(in_channels=1, out_channels=C, kernel_size=(1, 1)).to(x.device)
        # Res is B,C,W,H*H
        attention = torch.softmax(scores, dim=-1)
        out = torch.matmul(attention, value)  # (B, W, H, C)
        ...
        # Matrix multiplication => B,2C,H,1 AND B,2C, 1,W
        ...
        # x = x.permute(0, 3, 1, 2).contiguous()
        # print(f"Before conv: {y.shape}")
        # x = self.act(self.conv2d(x))
        # print(f"After act and conv: {y.shape}")
        # y1, y2, y3, y4 = self.forward_core(x)
        # assert y1.dtype == torch.float32
        # y = y1 + y2 + y3 + y4
        # print(f"After forward_core: {y.shape}")
        # y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        # print(f"After transpose: {y.shape}")
        y = self.out_norm(y)
